...
app.post('/api/products', requireAdmin, async (req, res) => {
    try {
        ...
        const { data, error } = await supabase
            .from('products')
            .insert([{ name, category, price, stock, image_url, description: description || '' }])
            .select();
        if (error) throw error;
...
app.put('/api/products/:id', requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const allowed = ['name', 'category', 'price', 'stock', 'image_url', 'description', 'discount_price', 'discount_active'];
        const updates = {};
...
// Update status pesanan (mis. "Menunggu Pembayaran" -> "Lunas" / "Diproses" / "Selesai")
